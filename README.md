...
**Étiquette** — 하나의 검색어를 여러 AI 에이전트가 각자 조사해 제안하고, 별도의 심사 에이전트가 근거를 비교해 하나의 답으로 압축해주는 멀티에이전트 쇼핑 가격비교 서비스.
...
- **OCR 검색어 추출 회귀**: 병합 시점 차이로 최신 수정 커밋이 main에서 누락된 것을 `git merge-base`로 진단, 동일 변경을 재적용해 복구
...
- **팀원 직접 푸시로 인한 이력 꼬임**: PR 없이 공용 브랜치에 직접 푸시된 커밋을 사후 검증(컴파일/부팅/동작 테스트)으로 안전성 확인 후 반영
- **순환 참조**: `history.py`가 필요로 하는 응답 유니온 타입을 `schemas.py`로 옮겨 `main.py`와의 순환 참조 해소
- **타임스탬프 단위 불일치**: 백엔드(epoch seconds)와 프론트(epoch ms) 간 단위 차이를 변환 함수로 통일

### 일일 스크럼 미팅 기록

> _Notion 스크럼 보드 링크로 대체 — `[Notion 링크 추가]`_

---

## 2️⃣ 일정 관리

> 아래 항목은 실제 팀 일정표/Notion 기준으로 채워 넣어야 하는 자리표시자입니다.

- **전체 프로젝트 기간**: 2026-08-04 ~ 2026-08-08 (커밋 이력 기준)
- **전체 프로젝트 일정표**: `[Notion/간트 차트 링크 추가]`
- **주요 마일스톤**: `[단계별 마감일 추가]`
- **주차별 스프린트 목표**: `[스프린트 목표 추가]`
- **개인별 세부 태스크 및 마감일**: `[태스크 보드 링크 추가]`
- **정기 미팅 스케줄**: `[주간 회의 일정 추가]`
- **출결 현황 및 부재 일정**: `[팀원 출결 기록 추가]`
- **일정 지연 사유 및 리스크 기록**: `[변수/리스크 기록 추가]`
- **최종 산출물 및 발표 마감일**: `[발표일 추가]`
...
[1️⃣ 팀원 구성 및 역할 분담](#팀원-구성-및-역할-분담) 참고.
...
- **소스 코드가 포함된 GitHub**: 이 저장소
- **Project 과정을 보여주는 기록 (Notion)**: `[Notion 링크 추가]`
- **발표 자료 (PDF / PPT)**: `[발표 자료 링크 추가]`
- **시연 영상**: `[시연 영상 링크 추가]`
