...
flowchart LR
    ...
    subgraph EXT["외부 서비스"]
        TAVILY["Tavily 검색 API<br/>(다나와 · 에누리 한정)"]
...
| 검색 | Tavily Search API (다나와 · 에누리로 도메인 한정) + 임베딩 기반 의미 유사도 검색 캐시 |
...
- **검색 도메인 15곳 → 다나와 · 에누리로 축소**: 15개 리테일러 각각 페이지 구조가 달라 스니펫만 보고 파싱하면 엉뚱한 상품 · 가격이 섞이는 문제가 있었음 — 다나와는 그 자체로 여러 판매처 가격을 한 페이지에서 비교해주는 가격비교 사이트라 도메인을 좁혀 일관성 · 정확도를 우선하고, 에누리를 이중화(다나와 차단 시 대비)로 추가
...
- **검색 데이터**: Tavily Search API를 통해 실시간으로 조회, 다나와 · 에누리 2곳으로 도메인 한정(원래 국내 리테일러 15곳이었으나, 사이트마다 페이지 구조가 달라 스니펫만으로 파싱하면 엉뚱한 상품이 섞이는 문제로 가격비교 사이트 1~2곳으로 축소)
...
- 검색 도메인을 다나와 · 에누리로 좁혀 신뢰도 낮은 결과 원천 차단(가격비교 사이트 특성상 여러 판매처를 한 페이지에서 일관된 구조로 비교 가능)
...
- 현재는 다나와 · 에누리 2곳으로 한정된 검색 범위를 점진적으로 확장할 여지가 있음
